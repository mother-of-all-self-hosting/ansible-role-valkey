...
#
# Tags look like `v<Valkey version>-<release>`:
#
# - if defaults/main.yml points at a Valkey version that has never been
#   released, the release counter restarts at 0 (`v9.1.1-0`)
# - otherwise the counter is incremented (`v9.1.1-1`), but only if something
...
version="$(sed -nE 's|^valkey_version:[[:space:]]*"?([^"[:space:]]+)"?.*$|\1|p' "$defaults_path" | head -n1)"

if [ -z "$version" ]; then
	echo >&2 "Could not determine the Valkey version from $defaults_path"
	exit 1
fi

# The version values do not carry a leading `v` (e.g. `9.1.1`), but the tags
# do (`v9.1.1-0`). Stripping any `v` before prepending one keeps this correct
